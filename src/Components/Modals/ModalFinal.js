...
export default function ModalFinal() {
    ...
    return (
        <div className={isCompletedModalVisible === true ? "modal-completed visible" : "modal-completed"}>
            <div className="icon-checked"><IconChecked alt="checked-icon" onClick={() => { setCompletedModalVisible(false); setOverlayVisible(false) }} /></div>
            ...
            <button className="modal-completed-btn" onClick={() => { setCompletedModalVisible(false); setOverlayVisible(false) }}>Got it!</button>
        </div>
    )
}